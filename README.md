...
- Photoshop 来源的蓝湖阴影会把 Distance、Angle、Spread 和 Blur 转换为 TMP Underlay 的 Offset、Dilate 和 Softness，并按项目的 Gamma/Linear 色彩空间转换效果颜色；描边、阴影扩展和模糊经过 TMP 视觉厚度校准，导入文字的 Character Spacing 固定为 `0`，文字内容不会写入 `<scale>` 标签。
